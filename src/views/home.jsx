...
function Home() {
  ...
  const handleConvert = () => {
    dispatch(getconvertCurrencies({ to: objData.currency2, from: objData.currency1, amount: objData.amount1 })).then((response) => {
      setObjData({
        ...objData,
        amount2: response.result,
      });
    });
  };
  const handleSwap = () => {
    const fromCurrency = objData.currency1;
    const toCurrency = objData.currency2;
    setObjData({
